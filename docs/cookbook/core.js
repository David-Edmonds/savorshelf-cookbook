...
(function(root) {
  ...
  function validRecipe(r) {
    ...
    if(r.country!==undefined&&(typeof r.country!=='string'||r.country.length>100))throw Error('Invalid country label.');
    if(r.ethnicCuisines!==undefined&&(!Array.isArray(r.ethnicCuisines)||r.ethnicCuisines.length>20||r.ethnicCuisines.some(x=>typeof x!=='string'||x.length>80)))throw Error('Invalid tradition labels.');
    return r;
  }
