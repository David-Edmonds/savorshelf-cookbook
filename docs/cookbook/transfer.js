...
function normalize(input,index=0){
 ...
 if(input.country!==undefined)r.country=cleanString(input.country,100);
 if(input.ethnicCuisines!==undefined){if(!Array.isArray(input.ethnicCuisines)||input.ethnicCuisines.length>20||input.ethnicCuisines.some(x=>typeof x!=='string'||x.length>80))throw Error('Invalid tradition labels.');r.ethnicCuisines=[...new Set(input.ethnicCuisines.map(x=>x.trim()).filter(Boolean))];}
