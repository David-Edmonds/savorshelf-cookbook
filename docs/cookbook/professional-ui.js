...
(function () {
  ...
  detail = function () {
    ...
    <section class="recipe-header"><div class="recipe-header-art">${recipeArt(r, 'detail-cover')}</div><div class="recipe-header-copy"><p class="eyebrow">${esc(r.category || 'Other')}${base.sourceStatus === 'rebuilt' ? ' · REBUILT RECIPE' : ''}</p><h1>${esc(r.title)}</h1><p class="recipe-intro">${esc(r.description)}</p><div class="recipe-facts"><span>${icon('clock')}${r.minutes ? 'About ' + C.fmt(r.minutes) + ' min' : 'Time not set'}</span><span>${icon('people')}${C.fmt(detailYield)} ${esc(r.yieldUnit || 'servings')}</span>${base.rating ? `<span aria-label="Your rating: ${base.rating} out of 5">★ ${base.rating}/5</span>` : ''}</div><p class="selected-version"><span class="small">Selected version</span><strong>${esc(version)}</strong>${actualModes ? `<span class="small dietary-meaning">${esc(actualModes)}</span>` : ''}${r.activeNotes?.length ? `<span class="small">${esc(summary)}</span>` : ''}</p></div></section>
    ...
    <div class="recipe-content-grid"><section class="panel ingredients-panel"><div class="section-label"><h2>Ingredients</h2><span id="ingredient-progress" class="small" role="status" aria-live="polite">${done} of ${ingredients.length} ready</span></div><div class="yield-control"><label for="yield"><strong>Make it for</strong><span class="small yield-unit">${esc(r.yieldUnit || 'servings')}</span></label><div class="gap"><button type="button" class="icon-btn" data-action="yield-down" aria-label="Decrease yield">−</button><input id="yield" type="number" min="1" max="1000" value="${detailYield}" aria-label="Recipe yield"><button type="button" class="icon-btn" data-action="yield-up" aria-label="Increase yield">+</button></div></div><div id="ingredient-list">${ingredients.map((i, n) => `<label class="ingredient"><input type="checkbox" data-ingredient="${n}" ${checkedIngredients.has(n) ? 'checked' : ''}><span class="ingredient-text"><span class="amount">${esc(K.ingredientText(i))}</span>${i.substitution ? `<span class="substitution">(${esc(noteText(i.substitution, r, detailYield))})</span>` : ''}</span></label>`).join('')}</div><details class="scaling-note"><summary>How serving adjustments work</summary><p class="small">Linked amounts in the steps follow your serving size. Times, temperatures and pan sizes do not multiply. Plain-text amounts in older recipes need linking in the editor. For part of an egg, beat it and measure the required fraction.</p></details></section>
    <section class="panel method-panel"><div class="section-label"><h2>Method</h2><span class="small">${r.steps.length} steps</span></div>${K.steps(r, detailYield).map((s, n) => `<div class="step"><span class="step-number">${n + 1}</span><div>${esc(s)}</div></div>`).join('')}</section></div>
