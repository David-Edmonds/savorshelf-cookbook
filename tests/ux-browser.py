...
with sync_playwright() as p:
 ...
 page.goto(f'http://127.0.0.1:{server.server_port}/cookbook/');expect(page.locator('#cookbook-count')).to_contain_text('1,458')
 ...
 page.locator('[data-action=uc-filter][data-filter=All]').click();expect(page.locator('#cookbook-count')).to_contain_text('1,458')
 ...
 page.locator('[data-action=uc-next]').click();expect(page.locator('#cookbook-count')).to_contain_text('2 / 61')
 ...
 expect(page.locator('#cookbook-count')).to_contain_text('2 / 61');page.wait_for_timeout(150)
 ...
 page.reload();expect(page.locator('#cookbook-count')).to_contain_text('2 / 61')
 ...
 assert page.evaluate("localStorage.getItem('our-table-v1')")==original
 page.evaluate('navigator.serviceWorker.ready');page.reload();page.context.set_offline(True);page.reload();expect(page.locator('#cookbook-count')).to_contain_text('1,458');assert page.evaluate("getComputedStyle(document.documentElement).getPropertyValue('--ux-ink').trim()")=='#19394a';page.context.set_offline(False)
