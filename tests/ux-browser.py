...
with sync_playwright() as p:
 ...
 page.goto(f'http://127.0.0.1:{server.server_port}/cookbook/');page.wait_for_function(f"window.RecipeCatalog?.count==={expected['count']}",timeout=90000);expect(page.locator('#cookbook-count')).to_contain_text(f"{visible_count:,}")
 ...
 page.locator('[data-action=uc-filter][data-filter=All]').click();page.wait_for_function(f"window.RecipeCatalog?.count==={expected['count']}",timeout=90000);expect(page.locator('#cookbook-count')).to_contain_text(f"{visible_count:,}")
 ...
 page.reload();expect(page.locator('#cookbook-count')).to_contain_text(f"2 / {(visible_count+23)//24}")
 ...
 assert page.evaluate("localStorage.getItem('our-table-v1')")==original
 page.evaluate('navigator.serviceWorker.ready');page.reload();page.context.set_offline(True);page.reload();page.wait_for_function(f"window.RecipeCatalog?.count==={expected['count']}",timeout=90000);expect(page.locator('#cookbook-count')).to_contain_text(f"{visible_count:,}");assert page.evaluate("getComputedStyle(document.documentElement).getPropertyValue('--ux-ink').trim()")=='#19394a';page.context.set_offline(False)
