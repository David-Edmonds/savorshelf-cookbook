...
for line in source.open(encoding='utf-8'):
    ...
    if row['id'] in aliases:
        assert aliases[row['id']]['bodySha256']==row['bodySha256']
        continue
    ...
    index.append([r['id'],r['title'],r['description'],r.get('category',''),r['minutes'],r['servings'],r.get('yieldUnit','servings'),r.get('country',''),r.get('cuisineTags',[]),[[i['name'],i['amount']] for i in ingredients],methods,chunk,bool(r.get('sourceWording')),r.get('sourceAliases',[])])
...
write(out/'repairs.json',{'recipes':{r['id']:r for r in rows if r.get('catalogId') in overrides},'aliases':{k:v['canonicalId'] for k,v in aliases.items()}})
